...
import emxModelList from '../emxlist';

const sortEmxModelList = Utils.sortModelList(emxModelList);
...
class EMXM1 extends Model.ElementXModel {
  ...
  isMatch(nodes: any[]) {
    ...
    // 所以默认直接返回true就好
    // 模型的匹配
    sortEmxModelList.forEach((matchModel: any) => {
      const groups = Utils.getGroupFromNodes(
        nodes,
      ...
      if (groups && groups.length > 0) {
        for (let i = 0; i < groups.length; i++) {
          ...
          if (!isMatched) {
            const bool = matchModel.isMatch(group);
    ...
    const that: any = this;
    // 对元素做一次abX的排序
    Utils.sortListByParam(that._matchDatas, 'abX', false);
    // 匹配完所有
    this._whenMatched();
