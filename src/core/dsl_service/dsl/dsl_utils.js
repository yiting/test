...
const utils = {
     ...
     * 可变节点模型获取节点的逻辑
     */
    getLineGroupFromNodes: function (nodes) {
        if (nodes.length == 0) {
            return [];
        }
        ...
        let regular2 = function (nodes) {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                for (let j = 0; j < res.length; j++) {
                    ...
                    if (((nd.abY + nd.height) >= arr.minH) && nd.abY <= arr.maxH) {
                        ...
                        arr.maxH = (nd.abY + nd.height) > arr.maxH ? (nd.abY + nd.height) : arr.maxH;
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                for (let j = 0; j < groups.length; j++) {
                    let node = groups[j];
                    if ((node.abY - 10) <= groups.minH && (node.abY + node.height + 10) >= groups.maxH) {
                        isOneLine = true;
                        break;
                    }
                }
        ...
        let regular3 = function (groups) {
            ...
            for (let i = 0; i < groups.length; i++) {
                let nodes = groups[i];
                utils.sortListByParam(nodes, 'abX');
