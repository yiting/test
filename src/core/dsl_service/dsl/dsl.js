...
let process = function(nodes, optimizeWidth, optimizeHeight, layoutType) {
  ...
  // 测试元素模型
  console.log('匹配的元素模型长度: ' + matchedElements.length);
  matchedElements.forEach(item => {
    if (item.modelName == 'emx-m1') {
      console.log(item);
    }
  });
 ...
 * @param {Json} json
 */
let getSymbolIdentity = function(json) {
...
let _matchModels = function(
  ...
  while (nowTimes < matchTimes) {
    ...
    // 需要匹配的节点
    let mNodes = Utils.getNodesFromSize(
      nodes,
      beginX,
      beginY,
      optimizeWidth,
