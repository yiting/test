...
class SymbolModel {
     ...
     */
    constructor(symbolList, ignoreList) {
        ...
        symbolList.forEach(symbol => {
            this._symbolDatas.push(new SymbolData(symbol.id, symbol.md5, symbol.template, symbol.style));
        });
    }
...
class MatchData {
     ...
     */
    _createFromElementX(renderData, matchDatas) {
        for (let i = 0; i < matchDatas.length; i++) {
            let mData = matchDatas[i];
            let rData = mData.getRenderData();
            rData.set('parentId', renderData.id);
            renderData.children.push(rData);
        }
    }
     ...
     * @param {ElementXModel} model 
     */
    _addElementXData(model) {
        // 可变节点元素模型拿到的是一系列基础ElementModel
        let ems = model._matchDatas;
        this.data = this._createRenderData(this.id, null, this.modelName, this.type, null, model);
        // 
        this._createFromElementX(this.data, ems);
    }
...
class SymbolData {
     ...
     */
    constructor(id, md5Value, tplStr, styleStr) {
        this._id = id || null;
 ...
 */
let jsonToMD5 = function (jsonData, ignoreList) {
    if (!jsonData) {
        return null;
    }
    ...
    let val = md5.update(strData).digest('hex');
