...
let matchModel = function (nodes, matchType, endY) {
    ...
    modelList.forEach(matchModel => {
        ...
        if (matchModel.getNumber() == 0) {
            // 可变节点模型的匹配
            // 获取可变节点元素模型的节点
            groups = Utils.getLineGroupFromNodes(nodes);
        }
        else {
            groups = Utils.getGroupFromNodes(nodes, matchModel.getTextNumber(), matchModel.getIconNumber(), matchModel.getImageNumber(), matchModel.getShapeNumber());
        }
