...
let process = function(dslTree, layoutType) {
  Loger.debug(`render.js [process]`);
  // 默认直接使用h5模板引擎输出
  TemplateData.reset();
  //注释掉toJSON方法
  let renderJSON = dslTree.getRenderData().toJSON();
  //let renderJSON = dslTree.getRenderData();
  con(renderJSON);
...
class Render {
  ...
  _setSimilarChild() {
    try {
      Object.keys(this._similarMap).forEach(similarId => {
        let group = this._similarMap[similarId];
   ...
   * @param {Array} nodes
   */
  _compareSimilar(nodes, _similarSourceId) {
    try {
      // 获取需要比对相似的节点
      let compareNodes = [];
      nodes.forEach(node => {
        compareNodes.push(
          ...node.children.filter(nd => {
            // 剔除绝对定位和已经是相似的节点
            if (
...
let _similarLogic = function(a, b, _nodeCache) {
  let aS = _nodeCache[a.similarParentId],
    bS = _nodeCache[b.similarParentId],
    aX = a.abX - aS.abX,
    aY = a.abY - aS.abY,
    bX = b.abX - bS.abX,
    bY = b.abY - bS.abY,
