...
let _buildTree = function(parent, data, layoutType) {
  try {
    ...
    Loger.error(
      `css_dom.js [_buildTree] ${e},params[parent.id:${parent &&
        parent.id},data.id:${data && data.id}]`,
    );
  }
};
/**
...
const cssPropertyMap = require('./css_property').map;
...
class CssDom {
  ...
  _isFixedHeight(node) {
    ...
    if (
      node.constraints['LayoutFixedHeight'] &&
      node.constraints['LayoutFixedHeight'] ==
  ...
  _calculateBoundary() {
    ...
    let isVertical =
      this.parent.constraints['LayoutDirection'] ==
      Constraints.LayoutDirection.Vertical;
    let left = this._calculateLeftBoundary(isVertical);
    let right = this._calculateRightBoundary(isVertical);
    this._calculateBoundaryConstraints(isVertical, left, right);
  }
  // 计算边界变更后的约束
  _calculateBoundaryConstraints(isVertical, left, right) {
  ...
  _supplementConstraints() {
    ...
    let isVertical = children.length > 0 && Utils.isVertical(children),
  ...
  getCssProperty(similarCss) {
    try {
      ...
      Loger.error(
        `css_dom.js [this.getCssProperty] ${e}, params[this.id:${
          this.id
        },${similarCss}]`,
      );
    }
    ...
    return (
      {
        ...
        End: 'flex-end',
        Center: 'center',
      }[this.constraints['LayoutAlignItems']] || null
    );
  }
  get justifyContent() {
    return (
      {
        ...
        End: 'flex-end',
        Center: 'center',
      }[this.constraints['LayoutJustifyContent']] || null
    );
  }
  //
  /* get boxAlign() {
  ...
  get overflow() {
    // if(this.children)
    if (
      this.width == Common.DesignWidth &&
      Utils.calRange(this.children).width > this.width
    ) {
      return 'auto';
    }
  ...
  get lineHeight() {
    if (this._hasText) {
      // 清洗行高，本应由数据源清洗
      let maxSize = Math.max(...this.styles.texts.map(t => t.size));
      let onlyOneLine = this._height / maxSize < 1.2;
      return (
  ...
  static getCssProperty(key, value) {
    if (!isNaN(value) && key != 'opacity' && key != 'zIndex') {
      // 数字的话进行单位转换
