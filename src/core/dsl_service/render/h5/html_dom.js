...
class HtmlDom {
  ...
  getHtmlStart(_layoutType) {
    if (_layoutType == Common.TestLayout) {
      ...
      return `<${this.getTag()} ${this.id} ${
        this.similarId ? 'sim=' + this.similarId : ''
      } ${modelName} ${this.getAttrClass()} ${this.getAttrs()}>${this.getContent()}`;
...
let _htmlDomTree, _cssDomMap, _similarCssDomMap;
