...
class Template {
  ...
  _parseObj(nd, parentTpl, isRoot) {
    ...
    if (!isRoot && renderData && renderData.modelName) {
      ...
      tplData = Template._assignObj(tplData, tplDataChild, nd);
    }
    // 遍历结构树
   ...
   * 特殊规则：小节点特殊属性覆盖大节点属性
   */
  static _assignObj(target = {}, source = {}, nd) {
    if (Object.keys(nd.attrs).includes(_SYMBOL.useTag)) {
      nd.tagName = source.tagName;
      // 删除标记
  ...
  set(refData, prop, value) {
    let target = Object.keys(refData).includes(prop)
      ? refData
      : refData.tplAttr;
  ...
  static parse(renderData, parentTpl, TemplateList) {
    // try {
    let ModelTpl =
      Template.getModelTemplate(TemplateList, renderData.modelName) || Template;
