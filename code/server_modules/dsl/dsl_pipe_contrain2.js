...
function calChildrenContrain(json) {
    ...
    let minLeftSpacing = Number.MAX_VALUE,
        minRightSpacing = Number.MAX_VALUE,
        minTopSpacing = Number.MAX_VALUE,
        minBottomSpacing = Number.MAX_VALUE;
    children.forEach((child, i) => {
        let prev = children[i - 1];
        minLeftSpacing = minLeftSpacing > child.x ? child.x : minLeftSpacing
        minRightSpacing = minRightSpacing > (json.width - child.x - child.width) ? (json.width - child.x - child.width) : minRightSpacing
        minTopSpacing = minTopSpacing > child.y ? child.y : minTopSpacing
        minBottomSpacing = minBottomSpacing > (json.height - child.y - child.height) ? (json.height - child.y - child.height) : minBottomSpacing
    ...
    if (isHorizontal) {
        ...
        if (Math.abs(minLeftSpacing - minRightSpacing) < Option.positionDeviation && maxHorizontalSpacing >= minLeftSpacing) {
            // 两端对齐
            cObj[CONTRAIN.LayoutJustifyContentBetween] = true;
        } else if (Math.abs(minLeftSpacing - minRightSpacing) < Option.positionDeviation && maxHorizontalSpacing < minLeftSpacing) {
            // 居中
            cObj[CONTRAIN.LayoutJustifyContentCenter] = true;
        } else if (minLeftSpacing < minRightSpacing) {
            // 居左
        ...
        if (minTopSpacing == minBottomSpacing && maxVerticalSpacing >= minTopSpacing) {
            // 两端对齐
            cObj[CONTRAIN.LayoutJustifyContentBetween] = true;
        } else if (minTopSpacing == minBottomSpacing && maxVerticalSpacing < minTopSpacing) {
            // 居中
            cObj[CONTRAIN.LayoutJustifyContentCenter] = true;
        } else if (minTopSpacing < minBottomSpacing) {
            // 居左
            cObj[CONTRAIN.LayoutJustifyContentStart] = true;
        } else {
            // 居右
        ...
        cObj[CONTRAIN.LayoutAbsolute] = true;
    }
    json.contrains = cObj;
