...
var ImageCombine = function(){
    ...
    this.combineNode = function(node,param){
        ...
        var  main = async () => {
            ...
            if(node.id.indexOf('_F71A_0689')>-1){//F061F4E3-EF2B-4C4A-A6F5-16D479DE4835 _FA04_1450_DD5E_F417_69BD_A45B_7421_CA02 145046AC-3A6C-41E0-8A5E-2107BC5BA70E 5C35_15C7_8C38_2DC8_BDBE_26B3_C862_35AD F41756E7-AFA1-4C53-803C-F4711B75B0C2
                console.log(111);
            }
            ...
            if(node.id.indexOf("_")== -1 && !(param && param.cantNative == true) && ( typeof node.isModified == "undefined" || node.isModified == false ) ){
                if(that.rootNode.id == node.id){
                    path = await this.combineShapeGroupNodeWithNative(node._origin,true);
                    ...
                    if(item._imageChildren && item._imageChildren.length>0 && item.id.indexOf("_")> -1){//&& item.isModified
                        ...
                        if(origin._class == "group" || origin._class == "shapeGroup" || origin._class == "symbolMaster" || origin._class == "oval" || origin._class == "bitmap" || origin._class == "rectangle" || origin._class == "shapePath"){
                            if(origin.layers && origin.layers.length == 1){
                                ...
                                // if(!hasMaskChildren){
                                    path = await this.combineShapeGroupNodeWithNative(item._origin);
    ...
    this.combineShapeGroupNodeWithNative = function(node,isReleaseFile){
        var  main = async () => {
            ...
            return new Promise(function (resolve, reject) {
                ...
                exec(command, function(a,b,c){
                    ...
                    if(isReleaseFile){
                        filePath = targetDir+node.do_objectID+".png";
                    }else{
                        filePath = outputDir+that._getTmpFileName(node.do_objectID)+".png";
                    }
                    try{
...
//         var  main = async () => {
//             // let rootNode = rootNode;
//             if(node.id.indexOf('F061F4E3-EF2B-4C4A-A6F5-16D479DE4835')>-1){//F061F4E3-EF2B-4C4A-A6F5-16D479DE4835 _FA04_1450_DD5E_F417_69BD_A45B_7421_CA02 145046AC-3A6C-41E0-8A5E-2107BC5BA70E 5C35_15C7_8C38_2DC8_BDBE_26B3_C862_35AD F41756E7-AFA1-4C53-803C-F4711B75B0C2
//                 console.log(111);
//             }
...
//             }
//             if(node.path){
//                 outputPath = outputDir+that._getTmpFileName(node.path);
//             }
//             var imageChildren = node._imageChildren || [node];
//             var isGroup = (node._imageChildren && node._imageChildren.length > 1) == true;
//             var path;
...
//                     var newNode = {};
//                     // console.log(outputPath);
//                     if(outputPath.indexOf("undefined")>-1){
//                         console.log(22);
//                     }
//                     outputImage.mosaic().write(outputPath,function (e) {
//                         if(e) {
...
//                     return new Promise(function (resolve, reject) { 
//                         outputPath = outputDir+that._getTmpFileName("m_"+maskItem.id)+".png";                  
//                         outputImage.mosaic().write(outputPath, function (e){
//                             if(e) {
//                                 console.log(e.message)
//                             }
...
//                                                 .in(outputPath)
//                                                 .mosaic().write(outputDir+that._getTmpFileName(maskItem.maskPath), function (e){
//                                                     if (err) throw err;
//                                                     resolve({
//                                                         "newNode" : newNode,
//                                                         "lastMaskedIndex": k
...
//         try{
//             fs.renameSync(outputDir+that._getTmpFileName(that.rootNode.path),outputDir+that.rootNode.path); 
//         }catch(e){
//             console.error(e);
...
//         }
//     },
//     hasMaskChildren:function(imageChildren){
//         var result = false;
//         for(var i=0,ilen=imageChildren.length;i<ilen;i++){
//             if(this._isQMask(imageChildren[i])){
//                 result = true;
