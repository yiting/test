    ...
    let getDrawDataFromNode = (imgNode) => {
        ...
        let count = (node) => {
            ...
            if (node.layers) {
                ...
                node.points && (function () {
                    data += JSON.stringify(node.points).toString()
                })();
            }
        }
        count(imgNode)        
        if(imgNode.style.fills){data+=JSON.stringify(imgNode.style.fills).toString()};
        if(imgNode.style.borders){data+=JSON.stringify(imgNode.style.borders).toString()};
        return data;
    }
    // 获取绘图基础数据
