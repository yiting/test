...
let TOSEEAPP = {
    ...
    $("#screen").load(function (event) {
      ...
      iframeBody
        .on("click", function (e) {
          ...
          //没有子节点，针对最细粒度节点操作
          if (
            chooseElementChildLen <= 2 &&
            hoverElement[0].nodeName != "HTML"
          ) {
            //1.dom节点属性信息
            //文本
            ...
            if (ElementTxt) {
              ...
              let ElementLetterSpaceReal = chooseElement.css("letter-space") ?
                parseFloat(
                  chooseElement.css("letter-space").replace("px", "")
                ) :
                0;
  ...
  hideChooseDom: function () {
    let iframeBody = $("#screen")
      ...
      .find("body");
    iframeBody
      .find(".choose-dom-style")
      .remove();
    iframeBody
      .find(".choose-dom-show")
      .removeClass("choose-dom-show");
  },
  /**
   * 显示右侧属性边框面板
   */
  ...
  initRectChosen: function () {
    let _this = this;
    rectChosen(document.querySelector('.screen-viewer'), document.querySelector('#screen'), _this.reAdjust.bind(this, currentArtboardId));
    ...
    btnGroup.addEventListener('click', function () {
      if (!_chosenNodeIds.length) return;
      btnGroup.setAttribute('disabled', '');
