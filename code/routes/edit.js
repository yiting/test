...
router.post("/getPageImgById", function(req, res, next) {
  ...
  let imgsResultJson = {},
    isImgsGenerate = true,
    artBoardResultArrLen = artBoardResultArr.length;

  //首先检测图片是否已生成
  //从已生成的结果中去取出
  for (let i = 0; i < artBoardResultArrLen; i++) {
    let artBoardResultOne = artBoardResultArr[i];
    if (
      artBoardId == artBoardResultOne.artBoardId &&
      projectUUID == artBoardResultOne.projectId &&
      artBoardResultOne.artBoardImgsArr &&
      generateImgsLen == artBoardResultOne.artBoardImgsArr.length
    ) {
      imgsResultJson.imgPaths = artBoardResultOne.artBoardImgsArr;
      //返回项目文件夹的id，可作为静态资源读取的路径拼接
      imgsResultJson.projectId = projectUUID;
      imgsResultJson.projectName = projectName;
      isImgsGenerate = false;
    }
  }
  ...
  if (isImgsGenerate) {
    combineImages(currentImgsList).then(info => {
      //生成的图片长度
      generateImgsLen = generateImgArr.length;
      //生成的图片放在结果数组里
      for (let i = 0; i < artBoardResultArrLen; i++) {
        let artBoardResultOne = artBoardResultArr[i];
        if (
          ...
          artBoardResultOne.artBoardImgsArr = generateImgArr;
          isImgsGenerate = false;
        }
      }
...
let getCurrentPageInfo = (req, res, next) => {
  ...
  let isHtmlGenerate = true,
    generateFileName;
  //检查是否存在某id
  let artBoardResultArrLen = artBoardResultArr.length;
  for (let i = 0; i < artBoardResultArrLen; i++) {
    let artBoardResultOne = artBoardResultArr[i];
    if (
      artBoardId == artBoardResultOne.artBoardId &&
      ...
      //页面存在，则不继续生成，返回
      generateFileName = artBoardResultOne.pageFileName;
  ...
  if (!isHtmlGenerate) {
    ...
    resultURL.htmlFileName = generateFileName;
    //输出对应处理好的artBoard数据
    //resultURL.htmlJson = h5Json;
    resultURL.isHtmlGenerate = false;
    //输出对应处理好的artBoard数据
    //resultURL.htmlJson = h5Json;
  ...
  //2018-11-01
  //首先判断当前数组中是否含对应projectID和artBoardId的对象，有的话就干掉再push
  for (let i = 0; i < artBoardResultArrLen; i++) {
    let artBoardResultOne = artBoardResultArr[i];
    if (
      artBoardId == artBoardResultOne.artBoardId &&
      ...
      artBoardResultArr.splice(index, 1);
    }
  }
  //然后再push
  artBoardResultArr.push({
    projectId: projectUUID,
    artBoardId: artBoardId,
    pageFileName: uploadTimeStamp
  });
    ...
    fs.readdir(pagesJsonDirectory, function(err, files) {
      ...
      try {
        ...
        if (currentDesignDom) {
          ...
          //2018-11-11
          let designJson = currentDesignDom.toList();
          if (!~artBoardList.findIndex(item => item.artBoardId === artBoardId))
            artBoardList.push({
              // Yone
              artboardId: artBoardId,
              designJson
...
router.post("/adjust", function(req, res) {
  ...
  let artboard = artBoardList.find(item => item.artboardId === artboardId);
  if (!artboard.designJson)
    res.send(
      JSON.stringify({
        status: "999999",
        msg: "修改失败"
  ...
  switch (operate) {
    case "1":
      {
        artboard.designJson = DSL.insertJson(
          artboard.designJson,
          req.body["nodeIds[]"]
        );
      }
